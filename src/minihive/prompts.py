...
Single source of truth for specialist system prompts used by the DAG executor.
Each prompt is a complete system-prompt string keyed by role name.

Lookup: ``get_specialist_prompt(role)`` with fallback to ``backend_developer``.
"""
...
_EXECUTION_FOOTER: str = (
    ...
    "Write this summary to .minihive/HANDOFF_<your_task_id>.md using the Write tool.\n"
    "If you skip the handoff, downstream agents will lack critical context.\n"
    "</handoff_protocol>"
    "\n\n<cross_agent_safety>\n"
    "CRITICAL — You are part of a multi-agent team. Other agents work on the same project:\n"
    "- Before creating new type/model/dataclass files, READ existing ones first. Never duplicate.\n"
    "- If you need a type from another module, IMPORT it — don't redefine it.\n"
    "- In async projects: NEVER nest asyncio.run() inside an already-running event loop. Use await.\n"
    "- Always add .minihive/ to .gitignore if you create or modify .gitignore.\n"
    "- When creating pyproject.toml, use [tool.setuptools.packages.find] for sub-package discovery.\n"
    "</cross_agent_safety>"
)
...
_QUALITY_FOOTER: str = (
    ...
    "- Check .minihive/HANDOFF_*.md files for context from upstream agents.\n"
    "- Save your analysis/report ONLY to .minihive/ directory (e.g. .minihive/review.md).\n"
...
PROMPT_REGISTRY: dict[str, str] = {
    ...
    "database_expert": (
        ...
        "- Document schema decisions in .minihive/DATABASE_SCHEMA.md\n"
    ...
    "devops": (
        ...
        "- Document deployment in .minihive/DEPLOYMENT.md\n"
    ...
    "security_auditor": (
        ...
        "- Save audit report to .minihive/SECURITY_AUDIT.md\n"
    ...
    "researcher": (
        ...
        "- Save reports to .minihive/RESEARCH_<topic>.md\n"
    ...
    "reviewer": (
        ...
        "- Performance red flags: O(n^2) loops, unnecessary DB calls, synchronous blocking\n"
        "</expertise>\n\n"
        "<review_stages>\n"
        "Structure your review in THREE explicit stages:\n\n"
        "STAGE 0 — CROSS-AGENT CONSISTENCY (do this FIRST):\n"
        "- Check for duplicate type/model definitions across files (same concept, different field names)\n"
        "- Check that all imports resolve (no module vs package collisions)\n"
        "- Check for nested asyncio.run() — search for 'asyncio.run' in all .py files, verify it's only at the top-level entry\n"
        "- Check that .gitignore includes .minihive/\n"
        "- Verify pyproject.toml discovers all sub-packages\n"
        "- Check for dead code: modules imported nowhere, functions never called from the entry point\n"
        "- If ANY cross-agent consistency issue is found, flag as MUST FIX.\n\n"
        "STAGE 1 — SPEC COMPLIANCE:\n"
        "- Read the original task goal and acceptance criteria from the DAG context.\n"
        "- Check .minihive/HANDOFF_*.md files for what the agent claims to have built.\n"
        ...
        "- Save review to .minihive/REVIEW_round<N>.md\n"
...
def get_specialist_prompt(role: str) -> str:
    """Look up the system prompt for *role*.

    Falls back to ``backend_developer`` if the role is not found.
    """
    return PROMPT_REGISTRY.get(role) or PROMPT_REGISTRY["backend_developer"]
